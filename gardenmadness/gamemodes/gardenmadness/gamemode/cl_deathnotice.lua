...
local hud_deathnotice_time = CreateConVar( "gma_deathnotice_time", "6", FCVAR_REPLICATED, "Amount of time to show death notice" )

-- These are our kill icons
local Color_Icon = Color( 255, 80, 0, 255 )
local NPC_Color = Color( 250, 50, 50, 255 )

killicon.Add( "weapon_plant_classic", "flicons/killicon_flower", Color( 255, 255, 255, 255 ) )
killicon.Add( "weapon_plant_lazor", "flicons/killicon_lazor", Color( 255, 255, 255, 255 ) )
killicon.Add( "weapon_plant_nuke", "flicons/killicon_nuke", Color( 255, 255, 255, 255 ) )

killicon.AddFont( "weapon_gma_smg",		"HL2MPTypeDeath",	"/",	Color_Icon )
killicon.AddFont( "weapon_gma_revolver",	"HL2MPTypeDeath",	".",	Color_Icon )
killicon.AddFont( "weapon_gma_ar2",		"HL2MPTypeDeath",	"2",	Color_Icon )
killicon.AddFont( "weapon_gma_pistol",	"HL2MPTypeDeath",	"-",	Color_Icon )

//killicon.AddFont( "weapon_flower_classic",	"FlowerWeaponKI",	",",	Color(255,255,255) )
...
local function RecvPlayerKilledByPlayer()
	...
	GAMEMODE:AddDeathNotice( attacker:Name(), attacker:GetGTeam(), inflictor, victim:Name(), victim:GetGTeam() )
...
local function RecvPlayerKilledSelf()
	...
	local victim = net.ReadEntity()
	if ( !IsValid( victim ) ) then return end
	GAMEMODE:AddDeathNotice( nil, 0, "suicide", victim:Name(), victim:GetGTeam() )
...
local function RecvPlayerKilledNPC()
	...
	GAMEMODE:AddDeathNotice( attacker:Name(), attacker:GetGTeam(), inflictor, victim, -1 )
...
function GM:AddDeathNotice( Attacker, team1, Inflictor, Victim, team2 )
	...
	else Death.color1 = gteams.GetColor(team1) end
	
	if ( team2 == -1 ) then Death.color2 = table.Copy( NPC_Color )
	else Death.color2 = gteams.GetColor(team2) end
...
local function DrawDeath( x, y, death, hud_deathnotice_time )
	...
	if death.color1 == nil then
		death.color1 = Color(255,255,255,255)
	end
	if death.color2 == nil then
		death.color2 = Color(255,255,255,255)
	end
